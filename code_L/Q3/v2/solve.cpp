...
void rhs(double t, const double y[10], double dy[10],
    ...
    double Pin = (p.F_amp * qd[0] + p.L_amp * qd[2]) * std::cos(p.omega * t);
    double Pd  = p.b_h * qd[0] * qd[0]
               + p.b_theta * qd[2] * qd[2]
               + C_L * (qd[1] - qd[0]) * (qd[1] - qd[0])
               + C_THETA * (qd[3] - qd[2]) * (qd[3] - qd[2]);
    dy[8] = Pin;
    dy[9] = Pd;
}
...
int main(int argc, char** argv) {
    ...
    // ======== L0 基准单元测试（v3 结果需求 指标 3 + 建模思路 L0 基准）========
    {
    ...
    // ======== 能量恒等式核验（继承 v1/v2 能量与耗散节，v2 式 Q3-18 线性版）========
    {
        ...
        for (auto& e : fr.energy) {
            double Pd = e.Pd_bh + e.Pd_bth + e.Pl + e.Pth;
            double resid = e.E_dir - (e.W_in - e.W_diss);
