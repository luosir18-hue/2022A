// Q3 v3 求解器（C++ 详细中文注释版）. Owner: 罗懿 (L, 建模手).
// 数据来源: 题目信息/A题/附件3.xlsx (ω=1.7152 对应行), 附件4.xlsx (全部数值从文件读取, 无手写数据)
// 模型依据: 模型/Q3/v3/数学模型.md 式(Q3-v3-1)~(Q3-v3-7)
//
// ======================== 系统概述 ========================
// 本程序求解问题三的四自由度牛顿—欧拉线性初值问题（L1 主模型）。
// 与 v1/v2 的拉格朗日非线性模型不同，v3 从牛顿第二定律（垂荡）和
// 欧拉转动方程（纵摇）独立建立小角度线性模型，得到常系数二阶系统：
//
//   M · ÿ + C · ẏ + K · y = f(t)
//
// 其中 M、C、K 均为 4×4 常系数矩阵，不随位置/角度变化。这是与非线性
// 模型的最大区别：无需在每一步重新组装质量矩阵、速度二次项和势能梯度。
//
// 广义坐标（绝对量，均从静平衡位置起算）: y = (z_f, z_o, θ_f, θ_o)ᵀ
//   z_f — 浮子质心绝对垂荡位移 (m, 向上为正)
//   z_o — 振子质心绝对垂荡位移 (m, 向上为正)
//   θ_f — 浮子绝对纵摇角 (rad, 中轴上端向波浪传播正方向倾斜为正)
//   θ_o — 振子/中轴绝对纵摇角 (rad)
// 一阶状态: Y = (yᵀ, ẏᵀ)ᵀ = (z_f,z_o,θ_f,θ_o, ż_f,ż_o,θ̇_f,θ̇_o)ᵀ 共 8 分量
//
// 注意状态顺序与 v1 的 (zf,θf,ξ,θo) 不同：v3 用振子绝对垂荡 z_o，
// 而 v1 用沿轴相对滑移 ξ；二者在小角度下满足 z_o = z_f + ξ。
//
// 控制方程分解（式 Q3-v3-1 ~ Q3-v3-5）:
//   垂荡块 M_h = [[M_f,0],[0,m_o]]、C_h = [[b_h+c_l,-c_l],[-c_l,c_l]]、
//           K_h = [[K_h+k,-k],[-k,k]]，外载 (F cosωt, 0)
//   纵摇块 M_p = [[J_f+m_o d², m_o d ℓ_e],[m_o d ℓ_e, I_o+m_o ℓ_e²]]、
//           C_p = [[b_θ+c_θ,-c_θ],[-c_θ,c_θ]]、
//           K_p = [[K_θ+k_θ-m_o g d, -k_θ],[-k_θ, k_θ-m_o g ℓ_e]]，外载 (L cosωt, 0)
//   其中 M_f=m_f+m_a、J_f=I_f+J_a、d<0（转轴在浮子质心下方）、ℓ_e>0（振子质心在转轴上方）。
//
// 关键物理: d<0 使浮子侧重力矩 -m_o g d 为稳定恢复项；ℓ_e>0 使振子侧
// -m_o g ℓ_e 为失稳项，但扭转弹簧 k_θ > m_o g ℓ_e 保证整体静态稳定。
//
// 求解方法: 预求 M 的逆（常系数，只求一次），右端函数为
//   ÿ = M⁻¹ [f(t) − C·ẏ − K·y]
// 再用 4 阶 Runge-Kutta (RK4, dt=0.001 s) 积分 40 个波浪周期；
// 0.2 s 只是题面输出采样间隔，不是积分步长。
//
// 输出文件 (全部写入本版本运行输出目录):
//   result3.xlsx            — 正式提交表 (9 列, 与 v1 列序一致)
//   result3.csv             — 同一数据 CSV 版 (UTF-8 BOM)
//   result3_key.csv         — 论文五时刻 (t=10,20,40,60,100 s)
//   result3_矩阵特性.csv     — 六个质量/阻尼/刚度块的最小特征值与性质判定
//   result3_几何约束检查.csv — 相对位移 u、相对角 φ、两类 PTO 瞬时功率
//   result3_线性化适用性.csv — 小角度/小滑移阈值判定
...
void build_matrices(const Params& p,
    ...
    double Mf = p.m_f + p.m_a;    // 浮子等效垂荡质量 M_f = m_f + m_a
    double Jf = p.I_f + p.J_a;    // 浮子等效纵摇惯量 J_f = I_f + J_a
    double d2 = p.d * p.d;        // d²
    double le2 = p.l_e * p.l_e;   // ℓ_e²
    double dle = p.d * p.l_e;     // d·ℓ_e

    for (int i = 0; i < 4; ++i)
        for (int j = 0; j < 4; ++j)
            M[i][j] = C[i][j] = K[i][j] = 0.0;   // 先清零，非块对角元保持为 0（无交叉水动力）

    // ---------- 垂荡块 M_h（式 Q3-v3-1）----------
    // 浮子方程: M_f·z_f'' + b_h·z_f' + K_h·z_f + c_l(z_f'-z_o') + k(z_f-z_o) = F cosωt
    // 振子方程: m_o·z_o'' + c_l(z_o'-z_f') + k(z_o-z_f) = 0
    M[0][0] = Mf;       // 浮子等效垂荡质量
    M[1][1] = p.m_o;    // 振子质量（无垂荡附加质量，附加质量只作用在浮子上）

    // ---------- 纵摇块 M_p（式 Q3-v3-4）----------
    // 浮子方程: (J_f+m_o d²)θ_f'' + m_o d ℓ_e·θ_o'' + (b_θ+c_θ)θ_f' − c_θ θ_o'
    //           + (K_θ+k_θ−m_o g d)θ_f − k_θ θ_o = L cosωt
    // 振子方程: m_o d ℓ_e·θ_f'' + (I_o+m_o ℓ_e²)θ_o'' − c_θ θ_f' + c_θ θ_o'
    //           − k_θ θ_f + (k_θ−m_o g ℓ_e)θ_o = 0
    M[2][2] = Jf + p.m_o * d2;     // 浮子绕 G_f 的等效惯量（含转轴偏置 d 的贡献）
    M[2][3] = p.m_o * dle;         // 轨道惯性耦合 m_o d ℓ_e
    M[3][2] = p.m_o * dle;         // 对称耦合项（作用—反作用，质量矩阵对称）
    M[3][3] = p.I_o + p.m_o * le2; // 振子绕 G_o 的惯量 + 平行轴 m_o ℓ_e²

    // ---------- 垂荡块 C_h ----------
    // 直线阻尼器 c_l 只依赖相对速度 (ż_o−ż_f)，两个物体上大小相等方向相反
    C[0][0] = p.b_h + C_L;   // 浮子：兴波阻尼 + 直线 PTO
    C[0][1] = -C_L;
    C[1][0] = -C_L;
    C[1][1] = C_L;           // 振子：只有直线 PTO

    // ---------- 纵摇块 C_p ----------
    // 旋转阻尼器 c_θ 只依赖相对角速度 (θ̇_o−θ̇_f)
    C[2][2] = p.b_theta + C_THETA;  // 浮子：兴波阻尼 + 旋转 PTO
    C[2][3] = -C_THETA;
    C[3][2] = -C_THETA;
    C[3][3] = C_THETA;              // 振子：只有旋转 PTO

    // ---------- 垂荡块 K_h^(2) ----------
    // 直线弹簧 k 只依赖相对位移 (z_o−z_f)；静水恢复 K_h 只作用在浮子
    K[0][0] = p.Kh + p.k;
    K[0][1] = -p.k;
    K[1][0] = -p.k;
    K[1][1] = p.k;
    ...
    // ---------- 纵摇块 K_p ----------
    // 扭转弹簧 k_θ 只依赖相对角 (θ_o−θ_f)；浮子侧含静水恢复 K_θ 与一阶重力矩
    K[2][2] = p.K_theta + p.k_theta - p.m_o * p.g * p.d;
    //             ↑ 注意 d<0，故 −m_o g d > 0 为稳定恢复项（转轴在质心下方）
    K[2][3] = -p.k_theta;
    K[3][2] = -p.k_theta;
    K[3][3] = p.k_theta - p.m_o * p.g * p.l_e;
    //             ↑ 注意 ℓ_e>0，故 −m_o g ℓ_e 为失稳项，但 k_θ 远大于 m_o g ℓ_e 保证静态稳定
}
...
void rhs(double t, const double y[10], double dy[10],
    ...
    const double* qd = y + 4;  // 广义速度 (ż_f, ż_o, θ̇_f, θ̇_o)

    // 波浪激励外载：垂荡力只作用在浮子(z_f)，纵摇力矩只作用在浮子(θ_f)
    double f[4] = {
        p.F_amp * std::cos(p.omega * t),  // F cosωt  → 浮子垂荡
        0.0,                              // 振子无直接外力
        p.L_amp * std::cos(p.omega * t),  // L cosωt  → 浮子纵摇
        0.0                               // 振子无直接外力矩
    };

    // 广义力 F_vec = f(t) − C·q̇ − K·q
    double Fv[4];
    for (int i = 0; i < 4; ++i) {
    ...
    // 加速度 q̈ = M⁻¹ · F_vec（M 为常系数，Minv 已预求一次）
